...
SELECT players.id,
	...
	seasons_stats.tm
FROM players
	INNER JOIN seasons_stats ON players.id = seasons_stats.player_id
WHERE players.id IN (0, 1, 2)
	AND seasons_stats.tm IN ('INO', 'TOT');
...
FROM seasons_stats
	LEFT JOIN players ON players.id = seasons_stats.player_id
WHERE players.id IN (0, 1, 2)
	AND seasons_stats.tm IN ('INO', 'TOT');